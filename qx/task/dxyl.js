...
!(async () => {
    try {
        ...
        if (interval == undefined) {
            $.setdata($.toStr(5), 'timeinterval')
            interval = '5'
        }


        if (hoursused >= 0) {
            minutesused = (thisminutes - lastminutes) + hoursused * 60
        } //上次查询的时间大于等于当前查询的时间
        else if (hoursused < 0 && lasthours <= 23) {
            minutesused = (60 - lastminutes) + (23 - lasthours + thishours) * 60 + thisminutes
        }
        //**********
        ...
        if (Tele_value == undefined) {
            $.setdata($.toStr(0), 'threshold');
            Tele_value = '0'
        }
        ...
        Query(Tele_body, loginerr).then(result => {
            ...
            if (thishours < 10) {
                tile_hour = '0' + thishours
            } else {
                tile_hour = thishours
            }
            if (thisminutes < 10) {
                tile_minute = '0' + thisminutes
            } else {
                tile_minute = thisminutes
            }
...
function Query_All(jsonData) {//原始量累计
    ...
    if (packge_switch == 'true') {
        for (var s in jsonData.RESULTDATASET) {
            ...
            if (k == 33120004 || k == 331202 || k == 351100 || k == 3511000 || k == 3312000 || k == 3312010 || k == 33120001 || k == 33140001 || k == 3322000 || k== 30220001 ||k==30120001)//判断定向
            {
...
function Notice(title, body, body1) {
    ...
    if (bark_key) {
        ...
        if ((effective != -1) && bark_other) {
            bark_other = `&${bark_other}`
        } else if ((effective == -1) && bark_other) {
            bark_other = `?${bark_other}`
        } else {
            bark_other = ''
        }
...
function ToSize(kbyte, s, l, t) {//字节转换s保留位数l是否空格t是否单位
    ...
    sizes = ["MB", "MB", "GB", "TB"];
    if (kbyte != 0) {
        i = Math.floor(Math.log(kbyte) / Math.log(k));
    }//获取指数
    else {
        i = 0
    }
